...
func TestRedisBridge_CrossInstanceDelivery(t *testing.T) {
	...
	instanceA := NewWorkflowService(newRunRepoStub(nil), nil, nil, nil, client, nil, nil, nil).(*workflowService)
	instanceB := NewWorkflowService(newRunRepoStub(nil), nil, nil, nil, client, nil, nil, nil).(*workflowService)
...
func TestRedisBridge_SingleInstanceDedup(t *testing.T) {
	...
	client := newRedisTestClient(t)
	svc := NewWorkflowService(newRunRepoStub(wf), &wfStubModelSvc{reply: "ok"}, nil, nil, client, nil, nil, nil)
