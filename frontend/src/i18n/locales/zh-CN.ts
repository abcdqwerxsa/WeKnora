export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'LuoSA Skill',
      subtitle: '通过 LuoSA REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 LuoSA Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 LuoSA 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 LuoSA 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    capabilityReadWorkflows: '读取工作流运行',
    capabilityReadWorkflowsHint: '查看工作流运行记录与详情，不能执行或修改定义',
    capabilityRunWorkflows: '执行工作流',
    capabilityRunWorkflowsHint: '执行已发布的工作流（含取消/续跑），不能读取或修改定义',
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 LuoSA 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 LuoSA 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      thinkingOn: '开启',
      thinkingOff: '关闭',
      thinkingHint: '关闭可避免思考内容混入输出；线上字段由模型的 thinking_control 决定。',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，LuoSA 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  auth: {
    ...
    registerSubtitle: '创建账户并开始使用 LuoSA',
    ...
    // 部门（注册时选择要加入的工作空间；后端校验 tenant.is_joinable）
    department: '所属部门',
    departmentPlaceholder: '请选择你要加入的部门',
    departmentRequired: '请选择所属部门',
    departmentLoadFailed: '部门列表加载失败，请稍后重试',
    departmentEmpty: '暂无可加入的部门，请联系管理员',
    departmentEmptyHint: '当前没有可加入的部门，提交后将为你创建个人工作空间',
    // 管理员待审核
    pendingApprovalTitle: '待审核用户',
    pendingApprovalSubtitle: '对自注册用户进行审核，批准后才能登录使用',
    pendingApprovalEmpty: '当前没有待审核的用户',
    pendingApprovalApprove: '批准',
    pendingApprovalReject: '拒绝',
    pendingApprovalRejectConfirm: '确定拒绝该用户的注册申请？该操作不可撤销',
    pendingApprovalApproved: '已批准',
    pendingApprovalRejected: '已拒绝',
    pendingApprovalFailed: '操作失败，请稍后重试',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是 LuoSA 的系统或部署配置。',
    helpAria: '沙箱密钥说明',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用 LuoSA 所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: 'LuoSA 标准模板',
      weknoraDockerImage: 'LuoSA 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: 'LuoSA 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 LuoSA 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有 LuoSA 标准模板时点「创建」；改 DNS 或镜像后点「删除并重建」。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建 LuoSA 标准模板，请稍后刷新查看状态',
      ...
      templateStepHint: '进入此步只会列出集群已有模板，不会自动创建。没有标准模板时点「创建 LuoSA 标准模板」；改 DNS 或镜像后点「删除并重建」。模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 LuoSA 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: 'LuoSA 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 LuoSA 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      ...
      cubeDnsServersHelp: '可选。写入 LuoSA 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'LuoSA 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 LuoSA 回收，会话继续时重建。留空按 1800 秒。',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，LuoSA 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 LuoSA',
...
,
  ...
  schedules: {
    ...
    nodeDesc: {
      Agent: '运行一轮受限知识检索的智能体', MCPTool: '调用已授权的 MCP 服务工具',
      Iteration: '对数组逐项执行循环体子图',
      ...
      HTTP: '调用内网 HTTP 接口',
      WebSearch: '调用配置的搜索 Provider', QuestionClassifier: '用模型把问题归类并路由', ParameterExtractor: '用模型从文本提取结构化参数'
    },
    title: '工作流',
    ...
    nameRequired: '请输入名称',
    description: '描述',
    descriptionPlaceholder: '这个工作流用来做什么？',
    status: { draft: '草稿', published: '已发布', archived: '已归档' },
    updatedAt: '更新时间',
    actions: '操作',
    edit: '编辑',
    delete: '删除',
    deleteConfirm: '确定删除工作流“{name}”？此操作不可撤销。',
    deleted: '工作流已删除',
    publish: '发布',
    unpublish: '取消发布',
    republish: '更新发布',
    ...
    actionFailed: '操作失败',
    publishConfirm: '确定发布“{name}”？发布后的运行执行当前快照，草稿可继续编辑。',
    deleteFailed: '删除工作流失败',
    empty: '暂无工作流',
    emptyHint: '创建第一个工作流开始使用',
    ...
    saved: '工作流已保存',
    nodes: { Start: '开始', LLM: 'LLM', Retrieval: '知识检索', Switch: '条件分支', Answer: '回答', Template: '模板变换', VariableAggregator: '变量汇聚', HTTP: 'HTTP 请求', DataOps: '数据查询', WebSearch: '网页搜索', QuestionClassifier: '问题分类', ParameterExtractor: '参数提取', Code: '代码', Iteration: '循环', Agent: '智能体', MCPTool: 'MCP 工具' },
    editor: {
      back: '返回',
      save: '保存',
      autoSavedAt: '已自动保存 {time}',
      runNode: '单节点运行',
      runNodeTitle: '调试节点：{name}',
      runNodeHint: '为每个上游节点填入输出 JSON 作为本节点的运行输入；跨节点参数引用将从这些值解析（仅调试用，不落库为真实上游结果）。',
      runNodeNoUpstream: '该节点没有上游节点，将不带注入输入直接运行。',
      runNodeNoOutput: '运行后此处显示该节点的输出。',
      outputViewTree: '树',
      outputViewSchema: 'Schema',
      schemaEmpty: '暂无可推导的字段结构。',
      schemaNoOutputYet: '尚未运行——以上为该节点声明输出的类型结构，运行后此处显示真实值与示例。',
      treeCountItems: '{n} 项',
      refCopyHint: '(点击复制)',
      treeCountFields: '{n} 字段',
      pin: '钉住数据',
      unpin: '取消钉住',
      ...
      outputViewJson: 'JSON',
      refCopied: '已复制 {ref}，可粘贴到参数提示词中',
      runNodeGo: '运行此节点',
      runNodeJsonPlaceholder: 'JSON 对象，例如 "参数名": "值"',
      runNodeFailed: '节点运行失败',
      nodeInputs: '节点输入',
      saveFailed: '保存工作流失败',
      importDsl: '导入 DSL',
      exportDsl: '导出 DSL',
      imported: 'DSL 已导入',
      importFailed: '无效的 DSL 文件',
      loadFailed: '加载工作流失败',
      notFound: '工作流不存在',
      properties: '节点属性',
      selectNode: '在画布上选择一个节点以编辑属性',
      prompt: '提示词',
      promptHint: '支持 节点ID＠参数 形式的引用，可用插入按钮',
      model: '模型',
      temperature: '温度',
      topK: 'Top K',
      cases: '分支条件',
      ...
      defaultBranch: '默认分支',
      template: '输出模板',
      modePointer: '指针模式（框选多选）',
      modeHand: '手模式（拖拽平移）',
      modeComment: '注释模式（点击画布添加便签）',
      notePlaceholder: '输入注释…',
      quickAdd: '添加下一个节点',
      ...
      maxTokensHint: '0 表示使用模型默认值',
      kbSelect: '知识库',
      kbSelectHint: '选择一个或多个知识库',
      queryTemplate: '检索问题模板',
      similarityThreshold: '相似度阈值',
      vectorThreshold: '向量阈值',
      keywordThreshold: '关键词阈值',
      thresholdHint: '留空表示不过滤',
      useRerank: '启用重排',
      rerankModel: '重排模型',
      rerankModelHint: '开启重排时必选',
      addOp: '添加操作',
      opGroup: '操作类型',
      opName: '操作',
      opUpper: '转大写',
      ...
      bodyTemplate: '请求体模板',
      timeout: '超时（秒）',
      httpIntranetHint: '仅可访问内网/私网地址',
      sql: 'SQL 查询',
      sqlPlaceholder: 'SELECT ... 支持用 $变量名 引用上游变量',
      ...
      issues: {
        ...
        multipleEntries: '存在 {count} 个入口节点：{names}。保存不受影响，但运行前需要只保留一个入口（或把多余入口接上游）',
        noTerminal: '缺少终端节点（无下游连线的节点）',
        unreachable: '节点 {name} 从入口不可达',
        staleRef: '节点 {name} 引用了不存在的 {ref}',
      badParent: '节点 {name} 的循环体归属 {parent} 不是循环节点',
      bodyEntries: '循环 {name} 的循环体必须有唯一入口节点（当前 {count} 个）',
      emptyBody: '循环节点 {name} 没有循环体成员',
      },
      formFields: '输入表单字段',
      fieldName: '字段名',
      fieldLabel: '显示名（可选）',
      ...
      variablesBtn: '变量',
      variablesDrawer: '工作流变量',
      variablesHint: '保存后通过 env.变量名 形式引用；重命名不会自动更新已有引用。',
      variablesName: '名称',
      variablesValue: '值',
      addVariable: '添加变量',
      searchProvider: '搜索 Provider',
      searchProviderHint: '留空使用默认 Provider',
      agentKbHint: '智能体将只在这些知识库中检索；留空则直接回答',
      platformAgent: '复用平台智能体',
      platformAgentHint: '选择已有智能体，不选则使用下方内联配置',
      platformAgentTip: '选中后节点复用该智能体的提示词、模型、知识库、MCP 等全部配置',
      mcpService: 'MCP 服务 ID',
      mcpTool: '工具名',
      mcpArgs: '参数（JSON）',
      mcpArgsPlaceholder: '渲染为 JSON 对象，值可含节点引用',
      mcpIntranetHint: '仅支持无交互授权的服务（none/api_key/bearer）；OAuth 服务会被拒绝',
      language: '语言',
      code: '脚本',
      ...
      classes: '分类',
      className: '类别名',
      classDesc: '类别描述（可选，帮助模型判断）',
      addClass: '添加类别',
      inputText: '输入文本',
      extractParams: '提取参数',
      fieldBool: '布尔',
      addExtractParam: '添加参数',
      errorHandling: '错误处理',
      onErrorFail: '失败并中止',
      onErrorContinue: '继续（使用默认输出）',
      onErrorRoute: '失败分支',
      defaultOutputs: '默认输出',
      errorRoute: '失败时路由到',
      errorRouteHint: '失败分支需在连线上同时保留唯一正常下游',
      retry: '重试',
      retryCount: '次数（0-5）',
      retryDelay: '间隔（毫秒）',
      items: '迭代数组',
      itemsPlaceholder: '渲染为 JSON 数组的模板，如 agg＠list 引用或 [1, 2, 3]',
      itemVar: '项变量名',
      outputRef: '收集输出引用',
      outputRefPlaceholder: '循环体终端输出，如 tpl＠text 引用',
      outputVar: '结果变量名',
      iterationHint: '循环体成员通过节点属性中的“循环体归属”选择；体内以 循环节点＠项变量 与 循环节点＠index 形式引用当前项与下标',
      bodyMembership: '循环体归属',
      bodyMembershipHint: '留空 = 外层图；选择后此节点加入对应循环节点的子图',
      },
    run: {
      open: '运行',
      title: '运行',
      ...
      cancel: '取消运行',
      cancelFailed: '取消运行失败',
      cancelledNotice: '运行已取消，未产生回答',
      resume: '续跑',
      resumeFailed: '续跑运行失败',
      resumedNotice: '已从断点续跑，已完成节点将被跳过',
      history: '运行历史',
      historyEmpty: '暂无运行记录',
