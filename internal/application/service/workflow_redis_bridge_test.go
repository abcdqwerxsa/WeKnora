...
func TestRedisBridge_CrossInstanceDelivery(t *testing.T) {
	...
	instanceA := NewWorkflowService(newRunRepoStub(nil), nil, nil, nil, client, nil, nil).(*workflowService)
	instanceB := NewWorkflowService(newRunRepoStub(nil), nil, nil, nil, client, nil, nil).(*workflowService)
