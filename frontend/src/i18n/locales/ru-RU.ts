export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'LuoSA Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API LuoSA — загрузки, URL, Markdown и поиск.',
      ...
      installCtaHint: 'Установка LuoSA Skill · откроется в новой вкладке',
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted LuoSA: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В LuoSA Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
...
capabilityReadWorkflows: 'Чтение запусков',
capabilityReadWorkflowsHint: 'Просмотр истории и деталей запусков; без выполнения и изменения определений',
capabilityRunWorkflows: 'Запуск процессов',
capabilityRunWorkflowsHint: 'Выполнение опубликованных процессов (вкл. отмену/возобновление); без чтения и изменения определений',
      ...
      hmacSecretDesc: 'Тот же secret, что в LuoSA; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
  ...
  agentEditor: {
    ...
    desc: {
      ...
      thinkingDefault: 'По умолчанию (решает модель)',
      thinkingOn: 'Включён',
      thinkingOff: 'Отключён',
      thinkingHint: 'Отключение не пропускает размышления в вывод; поле в запросе зависит от thinking_control модели.',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: 'Optional. LuoSA POSTs message_sent / message_received events to this HTTPS endpoint.',
  ...
  createChat: {
    title: 'Привет, я LuoSA — ваши знания всегда под рукой',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с LuoSA',
    ...
    // Подразделение (рабочее пространство для присоединения при регистрации; сервер проверяет tenant.is_joinable)
    department: 'Подразделение',
    departmentPlaceholder: 'Выберите подразделение, в которое хотите вступить',
    departmentRequired: 'Пожалуйста, выберите подразделение',
    departmentLoadFailed: 'Не удалось загрузить список подразделений. Повторите попытку позже',
    departmentEmpty: 'Сейчас нет подразделений, открытых для присоединения. Обратитесь к администратору.',
    departmentEmptyHint: 'Сейчас нет доступных для присоединения подразделений. После отправки формы для вас будет создано личное рабочее пространство.',
    // Администратор: очередь ожидающих одобрения
    pendingApprovalTitle: 'Ожидает одобрения',
    pendingApprovalSubtitle: 'Проверьте самостоятельно зарегистрированных пользователей до того, как они смогут войти',
    pendingApprovalEmpty: 'Нет пользователей, ожидающих одобрения',
    pendingApprovalApprove: 'Одобрить',
    pendingApprovalReject: 'Отклонить',
    pendingApprovalRejectConfirm: 'Отклонить эту заявку на регистрацию? Это действие нельзя отменить',
    pendingApprovalApproved: 'Одобрено',
    pendingApprovalRejected: 'Отклонено',
    pendingApprovalFailed: 'Не удалось выполнить операцию. Повторите попытку позже',
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки LuoSA.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте LuoSA; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине LuoSA — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      weknoraTemplateTitle: 'LuoSA standard template',
      weknoraDockerImage: 'LuoSA standard image',
      ...
      weknoraTemplateOverview: 'LuoSA provides the standard runtime. Templates are discovered after connecting and the standard one is created when missing.',
      weknoraTemplateDescription: 'Includes the Python, Node.js, CLI tools, workspace path, and non-root execution user expected by LuoSA skills.',
      ...
      templateLoadHint: 'Введите подключение к кластеру и обновите список. Если стандартного шаблона LuoSA нет, нажмите «Создать»; после смены DNS или образа — «Удалить и пересобрать».',
      templateLoadFailed: 'Failed to load templates',
      standardTemplateProvisioning: 'The LuoSA standard template is being created. Refresh shortly to see its status.',
      ...
      templateStepHint: 'На этом шаге только перечисляются шаблоны, уже есть в кластере; ничего не создаётся автоматически. Если стандартного шаблона нет, нажмите «Создать стандартный шаблон LuoSA»; после смены DNS или образа — «Удалить и пересобрать». Продолжить можно, когда шаблон готов.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — LuoSA пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Стандартный шаблон LuoSA',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон LuoSA будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон LuoSA. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
      ...
      dockerTlsCertPathHelp: 'Directory on the LuoSA host holding ca.pem, cert.pem and key.pem. Required for a remote daemon; certificates are mounted by the deployment, never stored here.',
      dockerIdleTtl: 'Idle reclaim (seconds)',
      dockerIdleTtlHelp: 'The Docker daemon has no idle timeout of its own. A container that runs no command for this long is reclaimed by LuoSA and rebuilt when the session continues. Empty means 1800 seconds.',
  ...
  agent: {
    ...
    taskWorkspace: {
      buttonTitle: 'Рабочая область задачи',
      title: 'Рабочая область задачи',
      tabArtifacts: 'Результаты',
      tabTerminal: 'Журнал выполнения',
      empty: 'В этой сессии пока нет результатов.',
      emptyTerminal: 'Журнал выполнения инструментов пуст.',
      collecting: 'Сохранение сгенерированных файлов…',
      refresh: 'Обновить',
      download: 'Скачать',
      previewBack: 'Назад к списку',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и LuoSA автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в LuoSA',
  ...
  chatHeader: {
    ...
    markdown: {
      ...
      attachHint: 'Вложения разбираются в контекст LLM (форматы платформы: PDF/Word/Excel/PPT/изображения и т.д.)',
...
,
  ...
  schedules: {
    ...
    cron: 'Выражение cron',
    query: 'Вход запуска (query)',
    queryPlaceholder: 'Запрос, передаваемый при каждом запуске',
    enabled: 'Включить при создании',
    create: 'Создать расписание',
    created: 'Расписание создано',
    createFailed: 'Операция с расписанием не выполнена',
    deleted: 'Расписание удалено',
    empty: 'Расписаний пока нет',
    enable: 'Включить',
    disable: 'Отключить',
    deleteConfirm: 'Удалить это расписание?',
    hint: 'Стандартный 5-полейный cron (мин час день месяц день недели); планировать можно только опубликованные процессы — снятие публикации останавливает запуски.',
  },
    palette: {
      title: 'Узлы',
      groups: {
    ...
    nodeDesc: {
      Agent: 'Выполнить один ход агента с поиском по базе знаний', MCPTool: 'Вызвать авторизованный инструмент MCP-сервиса',
      Iteration: 'Выполнить подграф тела для каждого элемента массива',
      ...
      WebSearch: 'Поиск через настроенные провайдеры', QuestionClassifier: 'Классифицирует вопрос с помощью LLM и маршрутизирует', ParameterExtractor: 'Извлекает структурированные параметры с помощью LLM'
    },
    title: 'Рабочие процессы',
    subtitle: 'Проектируйте детерминированные рабочие процессы агентов на холсте',
    create: 'Новый процесс',
    name: 'Название',
    namePlaceholder: 'Введите название процесса',
    nameRequired: 'Укажите название',
    description: 'Описание',
    descriptionPlaceholder: 'Что делает этот процесс?',
    status: { draft: 'Черновик', published: 'Опубликован', archived: 'В архиве' },
    updatedAt: 'Обновлён',
    actions: 'Действия',
    edit: 'Изменить',
    delete: 'Удалить',
    deleteConfirm: 'Удалить процесс «{name}»? Действие нельзя отменить.',
    deleted: 'Процесс удалён',
publish: 'Опубликовать',
unpublish: 'Снять с публикации',
republish: 'Обновить публикацию',
archive: 'В архив',
published: 'Рабочий процесс опубликован',
unpublished: 'Публикация снята',
archived: 'Рабочий процесс в архиве',
publishFailed: 'Не удалось выполнить публикацию',
actionFailed: 'Действие не выполнено',
publishConfirm: 'Опубликовать «{name}»? Запуски используют замороженный снимок; черновик продолжает редактироваться.',
    deleteFailed: 'Не удалось удалить процесс',
    empty: 'Процессов пока нет',
    emptyHint: 'Создайте первый процесс, чтобы начать',
    loadFailed: 'Не удалось загрузить список процессов',
    retry: 'Повторить',
    createTitle: 'Создать процесс',
    confirm: 'Подтвердить',
    cancel: 'Отмена',
    created: 'Процесс создан',
    createFailed: 'Не удалось создать процесс',
    saved: 'Процесс сохранён',
    nodes: { Start: 'Старт', LLM: 'LLM', Retrieval: 'Поиск', Switch: 'Ветвление', Answer: 'Ответ', Template: 'Шаблон', VariableAggregator: 'Агрегатор переменных', HTTP: 'HTTP-запрос', DataOps: 'SQL-запрос', WebSearch: 'Веб-поиск', QuestionClassifier: 'Классификатор вопросов', ParameterExtractor: 'Извлечение параметров', Code: 'Код', Iteration: 'Итерация', Agent: 'Агент', MCPTool: 'MCP-инструмент' },
    editor: {
      back: 'Назад',
      save: 'Сохранить',
      autoSavedAt: 'Автосохранено в {time}',
      runNode: 'Запустить этот узел',
      runNodeTitle: 'Отладка узла: {name}',
      runNodeHint: 'Введите выходной JSON каждого вышестоящего узла как вход для этого запуска; межузловые ссылки на параметры разрешаются из этих значений (только отладка, не сохраняется как реальный результат).',
      runNodeNoUpstream: 'У этого узла нет вышестоящих узлов — он запустится без внедрённых входов.',
      runNodeNoOutput: 'Запустите, чтобы увидеть вывод этого узла здесь.',
      outputViewTree: 'Дерево',
      outputViewSchema: 'Schema',
      treeCountItems: '{n} элем.',
      refCopyHint: '(нажмите, чтобы скопировать)',
      treeCountFields: '{n} полей',
      schemaEmpty: 'Пока нет выводимой структуры полей.',
      schemaNoOutputYet: 'Ещё не запускалось — выше объявленная структура выходов узла; запустите, чтобы увидеть реальные значения.',
      pin: 'Закрепить',
      unpin: 'Открепить',
      pinHint: 'Закреплено: полные запуски пропускают этот узел и используют замороженный вывод',
      startFormHint: 'Заполните форму и запустите: значения материализуются как выход узла Start для отладки нижестоящих узлов',
      pinnedBadge: 'Вывод закреплён (полные запуски пропускают узел)',
      outputViewTable: 'Таблица',
      outputTableRows: '{count} строк',
      outputViewJson: 'JSON',
      refCopied: 'Скопировано {ref} — вставьте в параметр/промпт',
      runNodeGo: 'Запустить этот узел',
      runNodeJsonPlaceholder: 'JSON-объект, например "параметр": "значение"',
      runNodeFailed: 'Не удалось запустить узел',
      nodeInputs: 'Входы узла',
      saveFailed: 'Не удалось сохранить процесс',
      importDsl: 'Импорт DSL',
      exportDsl: 'Экспорт DSL',
      imported: 'DSL импортирован',
      importFailed: 'Неверный файл DSL',
      loadFailed: 'Не удалось загрузить процесс',
      notFound: 'Процесс не найден',
      properties: 'Свойства',
      selectNode: 'Выберите узел на холсте, чтобы изменить свойства',
      prompt: 'Промпт',
      promptHint: 'Поддерживает ссылки nodeId＠param — кнопка вставки',
      model: 'Модель',
      temperature: 'Температура',
      topK: 'Top K',
      cases: 'Условия',
      caseTarget: 'Целевой узел',
      addCase: 'Добавить условие',
      defaultBranch: 'По умолчанию',
      template: 'Шаблон',
      modePointer: 'Указатель (рамка выделения)',
      modeHand: 'Рука (панорамирование)',
      modeComment: 'Комментарий (клик по холсту — заметка)',
      ...
      maxTokensHint: '0 — значение провайдера',
      kbSelect: 'Базы знаний',
      kbSelectHint: 'Выберите одну или несколько',
      queryTemplate: 'Шаблон запроса',
      similarityThreshold: 'Порог сходства',
      vectorThreshold: 'Порог векторов',
      keywordThreshold: 'Порог ключевых слов',
      thresholdHint: 'Пусто — без фильтрации',
      useRerank: 'Реранкинг',
      rerankModel: 'Модель реранкинга',
      rerankModelHint: 'Обязателен при включении',
      addOp: 'Добавить операцию',
      opGroup: 'Операция',
      opName: 'Операция',
      opUpper: 'Верхний регистр',
      opLower: 'Нижний регистр',
      opTrim: 'Обрезать пробелы',
      opReplace: 'Заменить',
      opRegex: 'Регулярное выражение',
      opFrom: 'Найти',
      opTo: 'Заменить на',
      opPattern: 'Шаблон',
      ...
      timeout: 'Таймаут (с)',
      httpIntranetHint: 'Только внутренние/приватные адреса',
      sql: 'SQL-запрос',
      sqlPlaceholder: 'SELECT ... переменные через $имя',
      ...
      deleteHint: 'Выберите узел или связь и нажмите Delete; двойной клик по связи удаляет её',
      ...
      issues: {
        ...
        multipleEntries: 'Узлов-входов: {count}: {names}. Сохранение не блокируется, но для запуска нужен ровно один вход',
      ...
      badParent: 'Родитель тела цикла {parent} узла {name} не является узлом итерации',
      bodyEntries: 'Тело цикла {name} должно иметь ровно один входной узел (найдено {count})',
      emptyBody: 'Узел итерации {name} не имеет участников тела',
      },
      formFields: 'Поля формы ввода',
      fieldName: 'Имя поля',
      fieldLabel: 'Подпись (необяз.)',
      fieldText: 'Строка',
      fieldParagraph: 'Абзац',
      fieldNumber: 'Число',
      fieldSelect: 'Список',
      ...
      variablesBtn: 'Переменные',
      variablesDrawer: 'Переменные рабочего процесса',
      variablesHint: 'После сохранения используйте формат env.имя; переименование не обновляет существующие ссылки.',
      variablesName: 'Имя',
      variablesValue: 'Значение',
      addVariable: 'Добавить переменную',
      searchProvider: 'Поисковый провайдер',
      searchProviderHint: 'Пусто — провайдер по умолчанию',
      agentKbHint: 'Агент ищет только в этих базах знаний; пусто — отвечает напрямую',
      platformAgent: 'Повторно использовать агента платформы',
      platformAgentHint: 'Выберите существующего агента; пусто — используется встроенная настройка ниже',
      platformAgentTip: 'Если выбран, узел повторно использует все настройки агента: промпт, модель, базы знаний, MCP и др.',
      mcpService: 'ID MCP-сервиса',
      mcpTool: 'Имя инструмента',
      mcpArgs: 'Аргументы (JSON)',
      mcpArgsPlaceholder: 'Рендерится в JSON-объект; значения могут содержать ссылки на узлы',
      mcpIntranetHint: 'Только сервисы с неинтерактивной аутентификацией (none/api_key/bearer); OAuth отклоняется',
      language: 'Язык',
      code: 'Скрипт',
      ...
      classDesc: 'Описание класса (необяз., помогает модели)',
      addClass: 'Добавить класс',
      inputText: 'Входной текст',
      extractParams: 'Извлекаемые параметры',
      fieldBool: 'Логическое',
      addExtractParam: 'Добавить параметр',
      errorHandling: 'Обработка ошибок',
      onErrorFail: 'Прервать запуск',
      onErrorContinue: 'Продолжить с значениями по умолчанию',
      onErrorRoute: 'Ветка ошибки',
      defaultOutputs: 'Выходные данные по умолчанию',
      errorRoute: 'Маршрут при сбое',
      errorRouteHint: 'Ветка ошибки требует ровно одного обычного исходящего ребра',
      retry: 'Повторные попытки',
      retryCount: 'Количество (0-5)',
      retryDelay: 'Задержка (мс)',
      items: 'Массив элементов',
      itemsPlaceholder: 'Шаблон, рендерящийся в JSON-массив, напр. ссылка agg＠list или [1, 2, 3]',
      itemVar: 'Переменная элемента',
      outputRef: 'Ссылка на собираемый вывод',
      outputRefPlaceholder: 'Вывод терминала тела, напр. ссылка tpl＠text',
      outputVar: 'Переменная результата',
      iterationHint: 'Участники тела добавляются селектором "тело цикла" на каждом узле; тело ссылается через iter＠item и iter＠index',
      bodyMembership: 'Тело цикла',
      bodyMembershipHint: 'Пусто = внешний граф; выбор добавляет узел в тело итерации',
      },
    run: {
      open: 'Запуск',
      title: 'Запуск',
      ...
      cancelledNotice: 'Запуск отменён — ответ не сформирован',
      resume: 'Продолжить',
      resumeFailed: 'Не удалось продолжить запуск',
      resumedNotice: 'Продолжено с контрольной точки — завершённые узлы пропускаются',
      ...
      attachments: 'Вложения запуска',
      attachFiles: 'Загрузить файлы',
      attachmentProcessing: 'Разбирается вложений: {n} — запуск разблокируется по готовности',
      attachmentWaitHint: 'Дождитесь окончания разбора вложений',
      attachmentUploadFailed: 'Не удалось загрузить вложение',
      nodeOutputs: 'Выходные данные узлов',
      stalePublished: 'Запуски используют опубликованный снимок, а не черновик. Переопубликуйте, чтобы применить изменения.',
     }
  }
}
