...
func formatSkillsMetadata(skillsMetadata []*skills.SkillMetadata, shellExecEnabled bool) string {
	...
	builder.WriteString("says nothing about whether the skill runs. The image's own `python3` does ship ")
	builder.WriteString("`python-docx`, `openpyxl` and `python-pptx`, so a standalone docx/xlsx/pptx ")
	builder.WriteString("generation script can just run with `shell_exec`'s `python3`\n")
